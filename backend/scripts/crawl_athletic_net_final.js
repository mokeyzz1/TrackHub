// Final Athletic.net Web Crawler - Gets EVERYTHING
...
async function crawlAthleticNetMeet(meetUrl, outputFile = 'crawled_meet_data_final.json') {
  console.log('🕷️  Athletic.net Web Crawler - FINAL VERSION\n');
  ...
  for (let i = 0; i < uniqueEvents.length; i++) {
    ...
    try {
      // Create new page for each event
        ...
        try {
          ...
          if (clicked) {
            ...
            if (tabName.toLowerCase().includes('result')) {
              ...
              console.log(`      ✅ ${tabData.length} rows (${parsed.length} parsed)`);
...
if (require.main === module) {
  ...
  const outputFile = process.argv[3] || 'crawled_meet_data_final.json';
