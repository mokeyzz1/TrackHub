// Full Athletic.net Web Crawler - Gets EVERYTHING from a meet
...
async function crawlAthleticNetMeet(meetUrl, outputFile = 'crawled_meet_data.json') {
  console.log('🕷️  Athletic.net Web Crawler\n');
  ...
  for (const eventLink of uniqueEvents) {
    ...
    try {
      ...
      const eventTabs = await page.evaluate(() => {
        const tabs = [];
...
if (require.main === module) {
  ...
  const outputFile = process.argv[3] || 'crawled_meet_data.json';
