...
export default function MeetDetailScreen() {
  ...
  const filteredEvents = meetData.events.filter((event) => {
    ...
    if (activeTab === 'running') {
      ...
      return name.includes('m ') || name.includes('mile') || name.includes('relay');
    }
    if (activeTab === 'field') {
      const name = event.name.toLowerCase();
      return name.includes('throw') || name.includes('jump') || name.includes('vault');
    }
    return true;
  });
  ...
  const getEventIcon = (eventName: string) => {
    ...
    if (name.includes('throw') || name.includes('shot') || name.includes('weight')) return '💪';
    ...
    <>
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.meetSubtitle}>Dec 13-14, 2024 • Concordia University</Text>
