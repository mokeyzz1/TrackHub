-- Exact rollback for 20260902_timing_platform_anet_live_completed_repair.
...
BEGIN
  ...
  IF archive_rows = 0 THEN
    RAISE NOTICE 'anet.live repair archive is absent; rollback is a safe no-op';
    RETURN;
  ELSIF archive_rows <> expected_rows THEN
    RAISE EXCEPTION 'anet.live rollback requires exactly %, found %', expected_rows, archive_rows;
  END IF;

  IF (SELECT count(DISTINCT source_pk)
        FROM ingest.fact_cleanup_archive
       WHERE operation_key = operation AND source_table = 'public.meets') <> expected_rows THEN
    RAISE EXCEPTION 'anet.live rollback archive contains duplicate meet ids';
  END IF;

  CREATE TEMP TABLE _anet_live_repair_rollback ON COMMIT DROP AS
  ...
  IF EXISTS (
    SELECT 1
      FROM _anet_live_repair_rollback r
      LEFT JOIN public.meets m USING (meet_id)
     WHERE m.meet_id IS NULL OR m.timing_platform IS DISTINCT FROM 'athletic_net'
  ) THEN
    RAISE EXCEPTION 'anet.live rollback found an unexpected current platform or missing meet';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'anet.live rollback did not restore the before-image exactly';
  END IF;

  DELETE FROM ingest.fact_cleanup_archive
   WHERE operation_key = operation AND source_table = 'public.meets';
  RAISE NOTICE 'anet.live rollback restored % rows', expected_rows;
END $$;
