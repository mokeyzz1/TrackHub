...
| Team/relay identity checkpoint | `teams` is still school-backed (3,516 rows; 3,509 legacy rows have no explicit name/type), source URLs are unique, and relay parent/leg links are structurally complete enough for bounded review. The focused repeat-leg gate finds 459 repeated athlete groups but 0 exact same-slot duplicates; all repeats use two different leg slots. The 96-ID relay conflict cohort is 214 canonical-match vs 300 mismatch legs, spanning multiple parents/meets. Two source-backed internal-ID mismatches had deterministic unique targets and were repaired with archived before-images; the remaining cases and all repeats remain held. | `TEAM_RELAY_IDENTITY_REVIEW_20260904.md`, `team_relay_identity_scan.sql`, `RELAY_LEG_DUPLICATE_REVIEW_20260904.md`, `relay_leg_duplicate_scan.sql`, `20260904210000_repair_source_backed_relay_athlete_links.sql`, `rollback_source_backed_relay_athlete_links.sql` |
...
| Meet source-ownership checkpoint | Existing private links were rolled up without adding an alias table: `66223` has 5 TFRRS links on meet 12914, `70965` has 20 Athletic.net links on 12937, `651507` splits 11 Athletic.net links to 12325 versus 11 TFRRS links to 12472, and `96401` has 21 linked of 24 source records on 12632 (three remain untyped/unlinked). No reassignment or merge was applied. | `MEET_SOURCE_OWNERSHIP_REVIEW_20260904.md`, `meet_source_ownership_scan.sql` |
...
| Function/RPC access checkpoint | The 17 application-owned functions have pinned search paths. Eleven ingest operations are explicit service-role/postgres only; public read RPCs are invoker-security; the sole security-definer function is the validated `register_push_token` RPC. Browser/public EXECUTE was revoked from the two application trigger helpers after a rollback-only trigger test; operator access remains, and platform-owned storage helpers were untouched. | `FUNCTION_ACCESS_REVIEW_20260904.md`, `function_access_scan.sql`, `20260904230000_harden_trigger_helper_execute_acl.sql` |
