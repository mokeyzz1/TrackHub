-- Emergency reversal for operation 20260902_consolidate_reviewed_school_duplicates.
...
BEGIN
  IF (SELECT count(*) FROM ingest.fact_cleanup_archive WHERE operation_key = operation) <> 11562 THEN
    RAISE EXCEPTION 'complete 11562-row cleanup archive is required for rollback';
  END IF;
  IF (SELECT count(*) FROM ingest.fact_cleanup_archive
       WHERE operation_key = operation AND source_table = 'public.schools.canonical') <> 17
     OR (SELECT count(*) FROM ingest.fact_cleanup_archive
          WHERE operation_key = operation AND source_table = 'public.schools.duplicate') <> 17
     OR (SELECT count(*) FROM ingest.fact_cleanup_archive
          WHERE operation_key = operation AND source_table = 'public.teams.duplicate') <> 33
     OR (SELECT count(*) FROM ingest.fact_cleanup_archive
          WHERE operation_key = operation AND source_table = 'public.athletes') <> 677
     OR (SELECT count(*) FROM ingest.fact_cleanup_archive
          WHERE operation_key = operation AND source_table = 'public.results') <> 9724
     OR (SELECT count(*) FROM ingest.fact_cleanup_archive
          WHERE operation_key = operation AND source_table = 'public.relay_results') <> 441
     OR (SELECT count(*) FROM ingest.fact_cleanup_archive
          WHERE operation_key = operation AND source_table = 'ingest.observations') <> 653 THEN
  ...
  IF (SELECT count(*)
       ...
       WHERE a.operation_key = operation AND a.source_table = 'public.schools.duplicate') = 17 THEN
    IF (SELECT count(*)
          FROM ingest.fact_cleanup_archive a
          JOIN public.results r ON r.result_id = a.source_pk::bigint
         WHERE a.operation_key = operation AND a.source_table = 'public.results'
           AND r.team_id = (a.row_data->>'team_id')::bigint) = 9724 THEN
      RETURN;
    END IF;
  ...
  UPDATE ingest.observations row
     SET target_team_id = (a.row_data->>'target_team_id')::bigint
    FROM ingest.fact_cleanup_archive a
   WHERE a.operation_key = operation AND a.source_table = 'ingest.observations'
     AND row.observation_id = a.source_pk::bigint;
  ...
  IF (SELECT count(*)
       ...
       WHERE a.operation_key = operation AND a.source_table = 'public.athletes'
         AND row.school_id = (a.row_data->>'school_id')::bigint) <> 677
     OR (SELECT count(*)
          FROM ingest.fact_cleanup_archive a JOIN public.results row
            ON row.result_id = a.source_pk::bigint
         WHERE a.operation_key = operation AND a.source_table = 'public.results'
           AND row.team_id = (a.row_data->>'team_id')::bigint) <> 9724
     OR (SELECT count(*)
          FROM ingest.fact_cleanup_archive a JOIN public.relay_results row
            ON row.relay_result_id = a.source_pk::integer
         WHERE a.operation_key = operation AND a.source_table = 'public.relay_results'
           AND row.team_id = (a.row_data->>'team_id')::integer) <> 441
     OR (SELECT count(*)
          FROM ingest.fact_cleanup_archive a JOIN ingest.observations row
            ON row.observation_id = a.source_pk::bigint
         WHERE a.operation_key = operation AND a.source_table = 'ingest.observations'
           AND row.target_team_id = (a.row_data->>'target_team_id')::bigint) <> 653 THEN
