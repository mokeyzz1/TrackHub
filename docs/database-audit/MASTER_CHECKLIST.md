...
Current focus: **MEET-03 — make production workflow schedules Central-time and DST safe**.
MEET-02 is complete in the branch. It is code-only: the app's meet tabs, detail page and latest-result
read now consume the lifecycle owner's stored status instead of independently guessing from the
device's local date. No historical rows were changed. Follow the checkpoint IDs below, with the owner's direction controlling
...
| MEET-02 | P1 | Use the stored meet lifecycle consistently across app tabs, meet details, related meets, latest-result reads and official ingestion | MEET-01 | Complete in branch 2026-09-09: frontend meet reads filter on `meets.status`; detail badges consume the same value, including cancelled; device-local date guessing and its incorrect single-day assumption are removed. 268 ingestion and 14 workflow/lifecycle tests pass. No schema or data changed. |
| MEET-03 | P1 | Preserve intended Central-time production schedules across CST/CDT without duplicate executions; manual runs must remain available | MEET-02 | In progress |
