...
| DATA-01-COLLEGIATE-PROFILES | P1 | Move the 1,184 still-Unattached confirmed collegiate athlete profiles to their reviewed schools, retaining Unattached per-result affiliations | DATA-01-COLLEGIATE-CATALOG | Open; cohort has zero cross-school profile conflicts. Requires exact before-images and affected-ID assertions. |
| DATA-01-COLLEGIATE-RESULTS | P1 | Repair source-proven competition affiliations for the confirmed collegiate cohort, including 52 Clark/Lane rows already routed to the wrong schools | DATA-01-COLLEGIATE-CATALOG | Open; 8,487 teamless results require result-level evidence. Profile/roster membership alone is not sufficient. |
