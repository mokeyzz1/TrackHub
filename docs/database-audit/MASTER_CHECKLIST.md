...
Current focus: **MEET-01 — make meet discovery, lifecycle status, and official results distinct**.
MEET-01 is complete in the branch. It is code-only: duplicate meet-discovery implementations are
consolidated, status transitions preserve real dates, and official ingestion accepts completed
meets only. No historical rows were changed. Follow the checkpoint IDs below, with the owner's direction controlling
