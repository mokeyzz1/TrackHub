...
test('all 106 reviewed active files retain their approved bytes and unique versions', () => {
  const approved = [
  ...
  assert.equal(approved.length, 106);
  assert.equal(new Set(approved.map(x => x.version)).size, 106);
