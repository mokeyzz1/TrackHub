...
test('all 104 reviewed active files retain their approved bytes and unique versions', () => {
  const approved = [
  ...
  assert.equal(approved.length, 104);
  assert.equal(new Set(approved.map(x => x.version)).size, 104);
