...
| DB-24 | Multi-event semantics | 70,764 `results` rows classified as points carry a typed time or distance; aggregate scores and component marks share the same `event_type_id`. | Model parent multi-event results and ordered components; exclude component-like rows from points PRs/rankings until deterministic mapping is proven. | Evidence captured 2026-09-04; held |
