...
async function getTopPerformances() {
  console.log('TOP PERFORMANCES - FEB 6-8, 2026 (Full Weekend)\n');

  const { data, error } = await supabase.rpc('get_weekly_performances', {
    p_start_date: '2026-02-06',
  ...
  const men = sorted.filter(p => {
    if (p.gender !== 'M' || seenMen.has(p.athlete_id)) return false;
    seenMen.add(p.athlete_id);
    return true;
  });

  console.log('\nTOP 10 MEN:');
  men.slice(0, 10).forEach((p, i) => {
    console.log(`${i+1}. ${p.full_name} - ${p.mark_raw} ${p.event_name} - ${p.waPoints} pts - ${p.school_name} (${p.division})`);
  });

  // Women
  const seenWomen = new Set();
  const women = sorted.filter(p => {
    if (p.gender !== 'F' || seenWomen.has(p.athlete_id)) return false;
    seenWomen.add(p.athlete_id);
    return true;
  });

  console.log('\nTOP 10 WOMEN:');
  women.slice(0, 10).forEach((p, i) => {
    console.log(`${i+1}. ${p.full_name} - ${p.mark_raw} ${p.event_name} - ${p.waPoints} pts - ${p.school_name} (${p.division})`);
  });
}
