-- Fix Weight Throw scoring coefficients
-- Previous coefficients were drastically undervaluing weight throws (25m = 357 pts instead of ~1200)
-- New coefficients calibrated to match Shot Put scoring for equivalent athletic achievement
...
DECLARE
  ...
  -- FIXED: Men's Weight Throw coefficients (was scoring ~357 for 25m, now ~1247)
  ...
  -- FIXED: Women's Weight Throw coefficients (was scoring ~372 for 25m, now ~1310)
...
BEGIN
  ...
  -- Cap at 1600 points to filter out formula artifacts
  ranked AS (
    SELECT *,
...
Fixed Weight Throw coefficients in Feb 2026 - was undervaluing throws by ~3x.
