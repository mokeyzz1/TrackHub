-- Fix event name matching to handle variations like "200 Meter Dash", "400 Meter Dash Invite", etc.
-- Uses pattern matching instead of exact matches
...
BEGIN
  ...
  WITH scored AS (
    SELECT
      ...
      -- Using pattern matching for flexible event name matching
      GREATEST(0, ROUND(
        CASE
          -- MEN'S TRACK EVENTS (use mark_seconds)
          WHEN a.gender = 'M' AND r.event_name ~* '^60\s*(Meters?|m)' AND r.mark_seconds IS NOT NULL THEN
            m_60m_a * r.mark_seconds * r.mark_seconds + m_60m_b * r.mark_seconds + m_60m_c
          WHEN a.gender = 'M' AND r.event_name ~* '^200\s*(Meters?|Meter\s*Dash|m)' AND r.mark_seconds IS NOT NULL THEN
            m_200m_a * r.mark_seconds * r.mark_seconds + m_200m_b * r.mark_seconds + m_200m_c
          WHEN a.gender = 'M' AND r.event_name ~* '^400\s*(Meters?|Meter\s*Dash|m)' AND r.mark_seconds IS NOT NULL THEN
            m_400m_a * r.mark_seconds * r.mark_seconds + m_400m_b * r.mark_seconds + m_400m_c
          WHEN a.gender = 'M' AND r.event_name ~* '^600\s*(Meters?|Meter|m|Yards?)' AND r.mark_seconds IS NOT NULL THEN
            m_600m_a * r.mark_seconds * r.mark_seconds + m_600m_b * r.mark_seconds + m_600m_c
          WHEN a.gender = 'M' AND r.event_name ~* '^800\s*(Meters?|Meter\s*Run|m)' AND r.mark_seconds IS NOT NULL THEN
            m_800m_a * r.mark_seconds * r.mark_seconds + m_800m_b * r.mark_seconds + m_800m_c
          WHEN a.gender = 'M' AND r.event_name ~* '^1000\s*(Meters?|m)' AND r.mark_seconds IS NOT NULL THEN
            m_1000m_a * r.mark_seconds * r.mark_seconds + m_1000m_b * r.mark_seconds + m_1000m_c
          WHEN a.gender = 'M' AND r.event_name ~* '(^Mile|^1\s*Mile)' AND r.mark_seconds IS NOT NULL THEN
            m_mile_a * r.mark_seconds * r.mark_seconds + m_mile_b * r.mark_seconds + m_mile_c
          WHEN a.gender = 'M' AND r.event_name ~* '^3000\s*(Meters?|m)' AND r.mark_seconds IS NOT NULL THEN
            m_3000m_a * r.mark_seconds * r.mark_seconds + m_3000m_b * r.mark_seconds + m_3000m_c
          WHEN a.gender = 'M' AND r.event_name ~* '^5000\s*(Meters?|m)' AND r.mark_seconds IS NOT NULL THEN
            m_5000m_a * r.mark_seconds * r.mark_seconds + m_5000m_b * r.mark_seconds + m_5000m_c
          WHEN a.gender = 'M' AND r.event_name ~* '^60\s*(Hurdles?|Meter\s*Hurdles?|mH)' AND r.mark_seconds IS NOT NULL THEN
            m_60mh_a * r.mark_seconds * r.mark_seconds + m_60mh_b * r.mark_seconds + m_60mh_c

          -- MEN'S FIELD EVENTS (use mark_meters)
          WHEN a.gender = 'M' AND r.event_name ~* '^High\s*Jump' AND r.mark_meters IS NOT NULL THEN
            m_hj_a * r.mark_meters * r.mark_meters + m_hj_b * r.mark_meters + m_hj_c
          WHEN a.gender = 'M' AND r.event_name ~* '^Pole\s*Vault' AND r.mark_meters IS NOT NULL THEN
            m_pv_a * r.mark_meters * r.mark_meters + m_pv_b * r.mark_meters + m_pv_c
          WHEN a.gender = 'M' AND r.event_name ~* '^Long\s*Jump' AND r.mark_meters IS NOT NULL THEN
            m_lj_a * r.mark_meters * r.mark_meters + m_lj_b * r.mark_meters + m_lj_c
          WHEN a.gender = 'M' AND r.event_name ~* '^Triple\s*Jump' AND r.mark_meters IS NOT NULL THEN
            m_tj_a * r.mark_meters * r.mark_meters + m_tj_b * r.mark_meters + m_tj_c
          WHEN a.gender = 'M' AND r.event_name ~* '^Shot\s*Put' AND r.mark_meters IS NOT NULL THEN
            m_sp_a * r.mark_meters * r.mark_meters + m_sp_b * r.mark_meters + m_sp_c
          WHEN a.gender = 'M' AND r.event_name ~* '^Weight\s*Throw|^WT$' AND r.mark_meters IS NOT NULL THEN
            m_wt_a * r.mark_meters * r.mark_meters + m_wt_b * r.mark_meters + m_wt_c

          -- WOMEN'S TRACK EVENTS
          WHEN a.gender = 'F' AND r.event_name ~* '^60\s*(Meters?|m)' AND r.mark_seconds IS NOT NULL THEN
            w_60m_a * r.mark_seconds * r.mark_seconds + w_60m_b * r.mark_seconds + w_60m_c
          WHEN a.gender = 'F' AND r.event_name ~* '^200\s*(Meters?|Meter\s*Dash|m)' AND r.mark_seconds IS NOT NULL THEN
            w_200m_a * r.mark_seconds * r.mark_seconds + w_200m_b * r.mark_seconds + w_200m_c
          WHEN a.gender = 'F' AND r.event_name ~* '^400\s*(Meters?|Meter\s*Dash|m)' AND r.mark_seconds IS NOT NULL THEN
            w_400m_a * r.mark_seconds * r.mark_seconds + w_400m_b * r.mark_seconds + w_400m_c
          WHEN a.gender = 'F' AND r.event_name ~* '^600\s*(Meters?|Meter|m|Yards?)' AND r.mark_seconds IS NOT NULL THEN
            w_600m_a * r.mark_seconds * r.mark_seconds + w_600m_b * r.mark_seconds + w_600m_c
          WHEN a.gender = 'F' AND r.event_name ~* '^800\s*(Meters?|Meter\s*Run|m)' AND r.mark_seconds IS NOT NULL THEN
            w_800m_a * r.mark_seconds * r.mark_seconds + w_800m_b * r.mark_seconds + w_800m_c
          WHEN a.gender = 'F' AND r.event_name ~* '^1000\s*(Meters?|m)' AND r.mark_seconds IS NOT NULL THEN
            w_1000m_a * r.mark_seconds * r.mark_seconds + w_1000m_b * r.mark_seconds + w_1000m_c
          WHEN a.gender = 'F' AND r.event_name ~* '(^Mile|^1\s*Mile)' AND r.mark_seconds IS NOT NULL THEN
            w_mile_a * r.mark_seconds * r.mark_seconds + w_mile_b * r.mark_seconds + w_mile_c
          WHEN a.gender = 'F' AND r.event_name ~* '^3000\s*(Meters?|m)' AND r.mark_seconds IS NOT NULL THEN
            w_3000m_a * r.mark_seconds * r.mark_seconds + w_3000m_b * r.mark_seconds + w_3000m_c
          WHEN a.gender = 'F' AND r.event_name ~* '^5000\s*(Meters?|m)' AND r.mark_seconds IS NOT NULL THEN
            w_5000m_a * r.mark_seconds * r.mark_seconds + w_5000m_b * r.mark_seconds + w_5000m_c
          WHEN a.gender = 'F' AND r.event_name ~* '^60\s*(Hurdles?|Meter\s*Hurdles?|mH)' AND r.mark_seconds IS NOT NULL THEN
            w_60mh_a * r.mark_seconds * r.mark_seconds + w_60mh_b * r.mark_seconds + w_60mh_c

          -- WOMEN'S FIELD EVENTS
          WHEN a.gender = 'F' AND r.event_name ~* '^High\s*Jump' AND r.mark_meters IS NOT NULL THEN
            w_hj_a * r.mark_meters * r.mark_meters + w_hj_b * r.mark_meters + w_hj_c
          WHEN a.gender = 'F' AND r.event_name ~* '^Pole\s*Vault' AND r.mark_meters IS NOT NULL THEN
            w_pv_a * r.mark_meters * r.mark_meters + w_pv_b * r.mark_meters + w_pv_c
          WHEN a.gender = 'F' AND r.event_name ~* '^Long\s*Jump' AND r.mark_meters IS NOT NULL THEN
            w_lj_a * r.mark_meters * r.mark_meters + w_lj_b * r.mark_meters + w_lj_c
          WHEN a.gender = 'F' AND r.event_name ~* '^Triple\s*Jump' AND r.mark_meters IS NOT NULL THEN
            w_tj_a * r.mark_meters * r.mark_meters + w_tj_b * r.mark_meters + w_tj_c
          WHEN a.gender = 'F' AND r.event_name ~* '^Shot\s*Put' AND r.mark_meters IS NOT NULL THEN
            w_sp_a * r.mark_meters * r.mark_meters + w_sp_b * r.mark_meters + w_sp_c
          WHEN a.gender = 'F' AND r.event_name ~* '^Weight\s*Throw|^WT$' AND r.mark_meters IS NOT NULL THEN
...
Uses regex pattern matching to handle event name variations like "200 Meter Dash Invite", "400 Meter Dash Higginbottom", etc.
