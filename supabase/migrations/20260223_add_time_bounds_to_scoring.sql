-- Add time/distance bounds to WA scoring to filter out injury times and invalid marks
-- A 51s 200m (injury/fall) should not appear as a top performance
...
BEGIN
  ...
  WITH scored AS (
    SELECT
      ...
      s.division::TEXT,
      -- Calculate WA points with time/distance bounds
      -- Returns NULL for times outside valid range (injury, DNF, etc.)
      GREATEST(0, ROUND(
        CASE
          -- MEN'S TRACK EVENTS (with max time bounds)
          WHEN a.gender = 'M' AND r.event_name IN ('60 Meters', '60m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 12 THEN
            m_60m_a * r.mark_seconds * r.mark_seconds + m_60m_b * r.mark_seconds + m_60m_c
          WHEN a.gender = 'M' AND r.event_name IN ('200 Meters', '200m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 35 THEN
            m_200m_a * r.mark_seconds * r.mark_seconds + m_200m_b * r.mark_seconds + m_200m_c
          WHEN a.gender = 'M' AND r.event_name IN ('400 Meters', '400m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 75 THEN
            m_400m_a * r.mark_seconds * r.mark_seconds + m_400m_b * r.mark_seconds + m_400m_c
          WHEN a.gender = 'M' AND r.event_name IN ('600 Meters', '600m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 120 THEN
            m_600m_a * r.mark_seconds * r.mark_seconds + m_600m_b * r.mark_seconds + m_600m_c
          WHEN a.gender = 'M' AND r.event_name IN ('800 Meters', '800m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 180 THEN
            m_800m_a * r.mark_seconds * r.mark_seconds + m_800m_b * r.mark_seconds + m_800m_c
          WHEN a.gender = 'M' AND r.event_name IN ('1000 Meters', '1000m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 240 THEN
            m_1000m_a * r.mark_seconds * r.mark_seconds + m_1000m_b * r.mark_seconds + m_1000m_c
          WHEN a.gender = 'M' AND r.event_name IN ('Mile', '1 Mile')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 360 THEN
            m_mile_a * r.mark_seconds * r.mark_seconds + m_mile_b * r.mark_seconds + m_mile_c
          WHEN a.gender = 'M' AND r.event_name IN ('3000 Meters', '3000m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 720 THEN
            m_3000m_a * r.mark_seconds * r.mark_seconds + m_3000m_b * r.mark_seconds + m_3000m_c
          WHEN a.gender = 'M' AND r.event_name IN ('5000 Meters', '5000m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 1200 THEN
            m_5000m_a * r.mark_seconds * r.mark_seconds + m_5000m_b * r.mark_seconds + m_5000m_c
          WHEN a.gender = 'M' AND r.event_name IN ('60 Hurdles', '60 Meter Hurdles', '60mH', '60m H')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 15 THEN
            m_60mh_a * r.mark_seconds * r.mark_seconds + m_60mh_b * r.mark_seconds + m_60mh_c

          -- MEN'S FIELD EVENTS (with min distance bounds)
          WHEN a.gender = 'M' AND r.event_name = 'High Jump'
               AND r.mark_meters IS NOT NULL AND r.mark_meters >= 1.0 THEN
            m_hj_a * r.mark_meters * r.mark_meters + m_hj_b * r.mark_meters + m_hj_c
          WHEN a.gender = 'M' AND r.event_name = 'Pole Vault'
               AND r.mark_meters IS NOT NULL AND r.mark_meters >= 2.0 THEN
            m_pv_a * r.mark_meters * r.mark_meters + m_pv_b * r.mark_meters + m_pv_c
          WHEN a.gender = 'M' AND r.event_name = 'Long Jump'
               AND r.mark_meters IS NOT NULL AND r.mark_meters >= 3.0 THEN
            m_lj_a * r.mark_meters * r.mark_meters + m_lj_b * r.mark_meters + m_lj_c
          WHEN a.gender = 'M' AND r.event_name = 'Triple Jump'
               AND r.mark_meters IS NOT NULL AND r.mark_meters >= 8.0 THEN
            m_tj_a * r.mark_meters * r.mark_meters + m_tj_b * r.mark_meters + m_tj_c
          WHEN a.gender = 'M' AND r.event_name = 'Shot Put'
               AND r.mark_meters IS NOT NULL AND r.mark_meters >= 5.0 THEN
            m_sp_a * r.mark_meters * r.mark_meters + m_sp_b * r.mark_meters + m_sp_c
          WHEN a.gender = 'M' AND r.event_name = 'Weight Throw'
               AND r.mark_meters IS NOT NULL AND r.mark_meters >= 5.0 THEN
            m_wt_a * r.mark_meters * r.mark_meters + m_wt_b * r.mark_meters + m_wt_c

          -- WOMEN'S TRACK EVENTS (with max time bounds - slightly more generous)
          WHEN a.gender = 'F' AND r.event_name IN ('60 Meters', '60m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 13 THEN
            w_60m_a * r.mark_seconds * r.mark_seconds + w_60m_b * r.mark_seconds + w_60m_c
          WHEN a.gender = 'F' AND r.event_name IN ('200 Meters', '200m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 40 THEN
            w_200m_a * r.mark_seconds * r.mark_seconds + w_200m_b * r.mark_seconds + w_200m_c
          WHEN a.gender = 'F' AND r.event_name IN ('400 Meters', '400m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 85 THEN
            w_400m_a * r.mark_seconds * r.mark_seconds + w_400m_b * r.mark_seconds + w_400m_c
          WHEN a.gender = 'F' AND r.event_name IN ('600 Meters', '600m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 140 THEN
            w_600m_a * r.mark_seconds * r.mark_seconds + w_600m_b * r.mark_seconds + w_600m_c
          WHEN a.gender = 'F' AND r.event_name IN ('800 Meters', '800m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 200 THEN
            w_800m_a * r.mark_seconds * r.mark_seconds + w_800m_b * r.mark_seconds + w_800m_c
          WHEN a.gender = 'F' AND r.event_name IN ('1000 Meters', '1000m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 270 THEN
            w_1000m_a * r.mark_seconds * r.mark_seconds + w_1000m_b * r.mark_seconds + w_1000m_c
          WHEN a.gender = 'F' AND r.event_name IN ('Mile', '1 Mile')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 420 THEN
            w_mile_a * r.mark_seconds * r.mark_seconds + w_mile_b * r.mark_seconds + w_mile_c
          WHEN a.gender = 'F' AND r.event_name IN ('3000 Meters', '3000m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 840 THEN
            w_3000m_a * r.mark_seconds * r.mark_seconds + w_3000m_b * r.mark_seconds + w_3000m_c
          WHEN a.gender = 'F' AND r.event_name IN ('5000 Meters', '5000m')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 1380 THEN
            w_5000m_a * r.mark_seconds * r.mark_seconds + w_5000m_b * r.mark_seconds + w_5000m_c
          WHEN a.gender = 'F' AND r.event_name IN ('60 Hurdles', '60 Meter Hurdles', '60mH', '60m H')
               AND r.mark_seconds IS NOT NULL AND r.mark_seconds <= 18 THEN
            w_60mh_a * r.mark_seconds * r.mark_seconds + w_60mh_b * r.mark_seconds + w_60mh_c

          -- WOMEN'S FIELD EVENTS (with min distance bounds)
          WHEN a.gender = 'F' AND r.event_name = 'High Jump'
               AND r.mark_meters IS NOT NULL AND r.mark_meters >= 1.0 THEN
            w_hj_a * r.mark_meters * r.mark_meters + w_hj_b * r.mark_meters + w_hj_c
          WHEN a.gender = 'F' AND r.event_name = 'Pole Vault'
               AND r.mark_meters IS NOT NULL AND r.mark_meters >= 2.0 THEN
            w_pv_a * r.mark_meters * r.mark_meters + w_pv_b * r.mark_meters + w_pv_c
          WHEN a.gender = 'F' AND r.event_name = 'Long Jump'
               AND r.mark_meters IS NOT NULL AND r.mark_meters >= 3.0 THEN
            w_lj_a * r.mark_meters * r.mark_meters + w_lj_b * r.mark_meters + w_lj_c
          WHEN a.gender = 'F' AND r.event_name = 'Triple Jump'
               AND r.mark_meters IS NOT NULL AND r.mark_meters >= 7.0 THEN
            w_tj_a * r.mark_meters * r.mark_meters + w_tj_b * r.mark_meters + w_tj_c
          WHEN a.gender = 'F' AND r.event_name = 'Shot Put'
               AND r.mark_meters IS NOT NULL AND r.mark_meters >= 5.0 THEN
            w_sp_a * r.mark_meters * r.mark_meters + w_sp_b * r.mark_meters + w_sp_c
          WHEN a.gender = 'F' AND r.event_name = 'Weight Throw'
               AND r.mark_meters IS NOT NULL AND r.mark_meters >= 5.0 THEN
...
Includes time/distance bounds to filter out injury times and invalid marks:
- 60m: max 12s (M), 13s (F)
- 200m: max 35s (M), 40s (F)
- 400m: max 75s (M), 85s (F)
- Field events: min distances to filter invalid marks
