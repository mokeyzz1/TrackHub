-- Route the existing public.meets.meet_url into the event-specific recovery queue.
--
-- meet_url is the raw timing/live URL and is not automatically a safe importer source. Keep it
-- as evidence, classify it, and only derive an importer candidate for TFRRS or the Athletic
-- family that the current event-scoped importer supports. Other timing vendors remain blocked
-- with explicit metadata so they can be assigned an adapter later instead of being mistaken for
-- missing links or scraped with the wrong parser.

COMMENT ON COLUMN ingest.event_recovery_queue.source_candidates IS
  'Source evidence for this meet. Includes meet_url plus provider/capability metadata; only validated TFRRS or Athletic-family URLs are importer candidates for the current 4x100 worker.';
