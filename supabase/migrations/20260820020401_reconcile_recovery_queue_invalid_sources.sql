-- Validate legacy source columns as well as generic meet_url values. Existing rows can contain a
-- timing-platform link in athletic_net_results_url from an earlier backfill, so COALESCE alone is
-- not sufficient. Rows without a supported source become blocked unless actively in progress.
