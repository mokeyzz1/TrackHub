-- Treat legacy meet_url values as a supported recovery source only when the host proves it is
-- TFRRS or athletic.net. A generic timing-site link must remain blocked for manual review.
  ...
  WITH target_meets AS (
    SELECT m.meet_id,
           ...
           COALESCE(m.athletic_net_results_url,
             CASE WHEN m.meet_url ~* '(athletic\.net|anet\.live|jdlfasttrack|blacksquirrel)' THEN m.meet_url END)
