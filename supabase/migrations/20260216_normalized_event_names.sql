-- Top Performances with CORRECT WA 2025 coefficients and normalized event names
  ...
  scored AS (
    SELECT
      ...
      CASE
        ...
        WHEN r.event_name ~* '^200\s*(Meters?|Meter\s*Dash|m)' THEN '200 Meters'
        ...
        WHEN r.event_name ~* '^60\s*(Hurdles?|Meter\s*Hurdles?|mH)' THEN '60 Hurdles'
    ...
    SELECT
      ...
      GREATEST(0, ROUND(
        CASE
          ...
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^60\s*(Meters?|m)$' AND scored.ms IS NOT NULL THEN
            m_60m_a * scored.ms * scored.ms + m_60m_b * scored.ms + m_60m_c

          -- MEN'S 200m
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^200\s*(Meters?|Meter\s*Dash|m|M)' AND scored.ms IS NOT NULL THEN
          ...
          -- MEN'S 400m
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^400\s*(Meters?|Meter\s*Dash|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- MEN'S 600m
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^600\s*(Meters?|Meter|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- MEN'S 800m
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^800\s*(Meters?|Meter\s*Run|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- MEN'S 1000m
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^1000\s*(Meters?|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- MEN'S 1500m
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^1500\s*(Meters?|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- MEN'S Mile
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '(^Mile|^1\s*Mile)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- MEN'S 3000m
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^3000\s*(Meters?|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- MEN'S 5000m
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^5000\s*(Meters?|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- MEN'S 60m Hurdles
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^60\s*(Hurdles?|Meter\s*Hurdles?|mH)' AND scored.ms IS NOT NULL THEN
            m_60mh_a * scored.ms * scored.ms + m_60mh_b * scored.ms + m_60mh_c

          -- MEN'S field events
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^High\s*Jump' AND scored.mm IS NOT NULL THEN
            m_hj_a * scored.mm * scored.mm + m_hj_b * scored.mm + m_hj_c
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^Pole\s*Vault' AND scored.mm IS NOT NULL THEN
            m_pv_a * scored.mm * scored.mm + m_pv_b * scored.mm + m_pv_c
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^Long\s*Jump' AND scored.mm IS NOT NULL THEN
            m_lj_a * scored.mm * scored.mm + m_lj_b * scored.mm + m_lj_c
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^Triple\s*Jump' AND scored.mm IS NOT NULL THEN
            m_tj_a * scored.mm * scored.mm + m_tj_b * scored.mm + m_tj_c
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^Shot\s*Put' AND scored.mm IS NOT NULL THEN
            m_sp_a * scored.mm * scored.mm + m_sp_b * scored.mm + m_sp_c
          WHEN scored.gender = 'M' AND scored.raw_event_name ~* '^Weight\s*Throw|^WT$' AND scored.mm IS NOT NULL THEN
            m_wt_a * scored.mm * scored.mm + m_wt_b * scored.mm + m_wt_c

          -- WOMEN'S 60m (indoor only)
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^60\s*(Meters?|m)$' AND scored.ms IS NOT NULL THEN
            w_60m_a * scored.ms * scored.ms + w_60m_b * scored.ms + w_60m_c

          -- WOMEN'S 200m
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^200\s*(Meters?|Meter\s*Dash|m|M)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- WOMEN'S 400m
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^400\s*(Meters?|Meter\s*Dash|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- WOMEN'S 600m
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^600\s*(Meters?|Meter|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- WOMEN'S 800m
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^800\s*(Meters?|Meter\s*Run|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- WOMEN'S 1000m
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^1000\s*(Meters?|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- WOMEN'S 1500m
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^1500\s*(Meters?|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- WOMEN'S Mile
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '(^Mile|^1\s*Mile)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- WOMEN'S 3000m
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^3000\s*(Meters?|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- WOMEN'S 5000m
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^5000\s*(Meters?|m)' AND scored.ms IS NOT NULL THEN
            CASE WHEN scored.is_indoor THEN
          ...
          -- WOMEN'S 60m Hurdles
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^60\s*(Hurdles?|Meter\s*Hurdles?|mH)' AND scored.ms IS NOT NULL THEN
            w_60mh_a * scored.ms * scored.ms + w_60mh_b * scored.ms + w_60mh_c

          -- WOMEN'S field events
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^High\s*Jump' AND scored.mm IS NOT NULL THEN
            w_hj_a * scored.mm * scored.mm + w_hj_b * scored.mm + w_hj_c
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^Pole\s*Vault' AND scored.mm IS NOT NULL THEN
            w_pv_a * scored.mm * scored.mm + w_pv_b * scored.mm + w_pv_c
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^Long\s*Jump' AND scored.mm IS NOT NULL THEN
            w_lj_a * scored.mm * scored.mm + w_lj_b * scored.mm + w_lj_c
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^Triple\s*Jump' AND scored.mm IS NOT NULL THEN
            w_tj_a * scored.mm * scored.mm + w_tj_b * scored.mm + w_tj_c
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^Shot\s*Put' AND scored.mm IS NOT NULL THEN
            w_sp_a * scored.mm * scored.mm + w_sp_b * scored.mm + w_sp_c
          WHEN scored.gender = 'F' AND scored.raw_event_name ~* '^Weight\s*Throw|^WT$' AND scored.mm IS NOT NULL THEN
...
'Gets top performances with CORRECT Official WA 2025 coefficients.
- Normalized event names (200 Meter Dash Invite → 200 Meters)
- Indoor/outdoor detection (based on 60m events in meet)
- Coefficients verified from github.com/jchen1/iaaf-scoring-tables
