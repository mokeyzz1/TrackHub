-- Preserve verified source URLs discovered by the private recovery worker when the
-- queue is refreshed. Canonical source fields on public.meets win for the same key;
-- queue-only discovery metadata and URLs remain durable between worker restarts.
