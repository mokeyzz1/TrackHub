-- Preserve unresolved source observations separately from public fact coverage. A meet can have
-- valid facts promoted and still require review for one or more ambiguous observations.

ALTER TABLE ingest.recovery_queue
  ADD COLUMN IF NOT EXISTS quarantined_observation_count integer NOT NULL DEFAULT 0;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conrelid = 'ingest.recovery_queue'::regclass
      AND conname = 'recovery_queue_quarantined_observation_count_check'
  ) THEN
    ALTER TABLE ingest.recovery_queue
      ADD CONSTRAINT recovery_queue_quarantined_observation_count_check
      CHECK (quarantined_observation_count >= 0);
  END IF;
END
$$;
...
BEGIN
  ...
  WITH target_meets AS (
    SELECT m.meet_id,
           ...
           COALESCE(q.quarantined_observation_count, 0)::integer AS quarantined_observation_count
    FROM public.meets m
