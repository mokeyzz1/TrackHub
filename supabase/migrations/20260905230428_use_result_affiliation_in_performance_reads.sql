-- Historical performance affiliation comes only from the represented result team.
-- Preserve unknown affiliations with LEFT JOIN; never use current athlete school as fallback.
SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '60s';
DO $guard$
BEGIN
  IF md5(pg_get_functiondef('public.get_top_performances(date,date,text,character,integer)'::regprocedure)) <> '2954dc8d3e54738ab1c52b9bf96d664a'
     OR md5(pg_get_functiondef('public.get_weekly_performances(date,date,text,integer)'::regprocedure)) <> '98436364d358c7e8b260619ef946196d' THEN
    RAISE EXCEPTION 'Performance function definitions changed since reviewed before-image';
  END IF;
END;
$guard$;


CREATE OR REPLACE FUNCTION public.get_top_performances(p_start_date date, p_end_date date, p_division text DEFAULT NULL::text, p_gender character DEFAULT NULL::bpchar, p_limit integer DEFAULT 100)
 RETURNS TABLE(athlete_id integer, full_name text, gender character, event_name text, mark_raw text, mark_seconds numeric, mark_meters numeric, date date, meet_name text, meet_id integer, place smallint, school_name text, division text, wa_points integer)
 LANGUAGE plpgsql
 STABLE
...
BEGIN
  ...
  scored AS (
    ...
    LEFT JOIN teams represented_team ON r.team_id = represented_team.team_id
  LEFT JOIN schools s ON represented_team.school_id = s.school_id
    WHERE r.date BETWEEN p_start_date AND p_end_date
...
CREATE OR REPLACE FUNCTION public.get_weekly_performances(p_start_date date, p_end_date date, p_division text DEFAULT NULL::text, p_limit integer DEFAULT 5000)
 RETURNS TABLE(athlete_id integer, full_name text, gender character, event_name text, mark_raw text, mark_seconds numeric, mark_meters numeric, date date, meet_name text, meet_id integer, place smallint, school_name text, division text)
 LANGUAGE plpgsql
 STABLE
 SET search_path TO 'public', 'pg_temp'
AS $function$
