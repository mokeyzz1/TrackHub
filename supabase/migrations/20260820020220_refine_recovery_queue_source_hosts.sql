-- Keep recovery candidates aligned with the source adapters that actually exist.
-- blacksquirreltiming and jdlfasttrack are timing platforms, not athletic.net aliases; they must
-- remain blocked/future-adapter work instead of being presented as supported athletic.net rows.
...
BEGIN
  ...
  WITH target_meets AS (
    SELECT m.meet_id,
           COALESCE(m.tfrrs_url,
             CASE WHEN m.meet_url ~* '(tfrrs\.org|tfrrs)' THEN m.meet_url END) AS tfrrs_url,
           COALESCE(m.athletic_net_results_url,
             CASE WHEN m.meet_url ~* '(athletic\.net|anet\.live)' THEN m.meet_url END)
             AS athletic_net_results_url,
           m.wa_results_url,
           m.meet_url,
